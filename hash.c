...
void hash_free_nodes(hash_t *hash)
{
    ...
    while (0!=(node = hash_scan_next(&hs))) {
...
void hash_insert(hash_t *hash, hnode_t *node, const void *key)
{
    ...
    /* bmt - 9/8/03
       IMO, it does not make sense to have an arbitrary
       limit on the number of nodes in the hash table if
       nothing in the implementation requires it.
       Note that consequently, is_full will always return
       false.
     */
#if 0
